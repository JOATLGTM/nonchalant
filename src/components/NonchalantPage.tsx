...
export function NonchalantPage() {
  ...
  const goToForm = useCallback(() => {
    if (!introRef.current || !formContainerRef.current || !formFieldsRef.current) return;
  ...
  const goToConfirmation = useCallback(() => {
    if (!formContainerRef.current || !confirmRef.current || !confirmContentRef.current) return;
    ...
    const hairline = confirmContentRef.current.querySelector('.confirm-hairline');
    if (hairline) {
      tl.fromTo(
        hairline,
        { scaleX: 0 },
  ...
  const goToIntro = useCallback(() => {
    ...
    tl.set(introRef.current, { display: 'flex', opacity: 0, filter: 'blur(0px)' });
    tl.to(introRef.current, {
      opacity: 1,
      duration: 0.8,
      ease: 'power2.out',
  ...
  // ── Form validation ──
  const validateField = useCallback((name: keyof SignupData, value: unknown): string | undefined => {
  ...
  useEffect(() => {
    if (pageState === 'form') {
      const firstInput = formContainerRef.current?.querySelector('input[name="full_name"]') as HTMLInputElement | null;
      if (firstInput) {
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen w-full overflow-hidden" style={{ background: 'var(--bg)' }}>
      {/* Background layers */}
      <div
      ...
      >
        <div className="w-full max-w-[440px] px-6 py-12">
          <div ref={formFieldsRef}>
            ...
            >
              Curated by Chef Marcos Ju&aacute;rez &middot; Cocktails by Hao Ma. Tell us who's coming and we'll be in touch.
            </p>
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="mb-5">
                ...
                />
                {errors.full_name && (
                  <p id="full_name-error" className="form-error" role="alert">{errors.full_name}</p>
                )}
              </div>
              ...
              <div className="mb-5">
                ...
                />
                {errors.phone && (
                  <p id="phone-error" className="form-error" role="alert">{errors.phone}</p>
                )}
              </div>
              ...
              <div className="mb-6">
                <label htmlFor="party_size" className="form-label">
                  Party size <span style={{ color: 'var(--accent)' }}>*</span>
                </label>
                <input
                  id="party_size"
                  name="party_size"
                ...
                />
                {errors.party_size && (
                  <p id="party_size-error" className="form-error" role="alert">{errors.party_size}</p>
                )}
              </div>
              ...
              {/* General error */}
              {errors.general && (
                <p className="form-error mb-4 text-center" role="alert">{errors.general}</p>
              )}
      ...
      <div
        ...
        aria-live="polite"
      >
        <div ref={confirmContentRef} className="text-center px-6 max-w-[360px]">
          {/* Checkmark */}
          <span
            style={{
              fontFamily: "'Cinzel', serif",
          ...
          >
            We'll reach out with dates for the next NONCHALANT supper club.
          </p>
        </div>
      </div>
    </div>
